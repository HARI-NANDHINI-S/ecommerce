...
## Threat Model
- **Secrets exposure** – Supabase `service_role` key and Razorpay secret are stored only in server‑side Edge Functions and never sent to the browser.
...
```json
{
  "headers": [{
    "source": "/(.*)",
    "headers": [
      {"key":"Content‑Security‑Policy","value":"default-src 'self'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://*.supabase.co https://api.razorpay.com;"},
...
- Razorpay webhooks include signature verification; failed attempts are logged.
- Vercel provides request logs; set alerts for abnormal traffic spikes.

## Incident Response
1. Revoke compromised keys immediately (Supabase service‑role, Razorpay secret).
